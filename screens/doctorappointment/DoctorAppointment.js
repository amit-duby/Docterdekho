...
//     TextInput
// } from 'react-native';
// import { __getApiData, __postApiData } from "../../utils/api"; // Assuming you have the __postApiData function for POST requests
// import { Colors, Fonts, Sizes } from "../../constants/styles";
// import { MaterialIcons } from "@expo/vector-icons";
...
//     const [loading, setLoading] = useState(true);
//     const [error, setError] = useState(null);
//     const [selectedBooking, setSelectedBooking] = useState(null); // To store selected booking data for updating
//     const [status, setStatus] = useState('');
//     const [bookingNumber, setBookingNumber] = useState('');
//     const [modalVisible, setModalVisible] = useState(false);
...
//     const fetchBookings = () => {
//         __getApiData('/api/booking/my_bookings')  // API endpoint
//             .then((res) => {
//                 if (res?.response?.response_code === "200" && res?.data?.aaData) {
//                     setBookings(res?.data?.aaData);  // Store the bookings data
//                 } else {
//                     setError('No bookings found');  // If no bookings found
//                 }
//                 setLoading(false);  // Stop loading
//             })
//             .catch((error) => {
//                 setError('Failed to fetch bookings');  // Handle error
//                 setLoading(false);  // Stop loading
//             });
//     };
...
//     // Function to handle updating the booking status and number
//     const updateBooking = () => {
...
//         console.log("Sending data to the API:", updatedData);
...
//                     fetchBookings(); // Refresh the booking list after updating
...
//                 } else {
//                     alert('Network or request error: ' + error.message);
//                 }
//             });
//     };
...
//         <View style={styles.cardContainer}>
//               {/* Button to open the modal for updating */}
//               <TouchableOpacity onPress={() => openUpdateModal(item)}>
//                 <Text style={styles.updateButton}>Update Booking</Text>
//             </TouchableOpacity>
//             <Text style={styles.patientName}>PatientName: {item.customer_name || 'N/A'}</Text>
//             <Text style={styles.patientName}>Patient ID: {item.id || 'N/A'}</Text>
//             <Text style={styles.serviceName}>Booking No: {item.booking_no || 'N/A'}</Text>
//             <Text style={styles.status}>Status: {item.status || 'N/A'}</Text>
//             <Text style={styles.date}>Date: {item.booking_date || 'N/A'}</Text>
//             <Text style={styles.time}>Time: {item.time || 'N/A'}</Text>
//         </View>
//     );

//     // Function to open the modal and set selected booking data
//     const openUpdateModal = (item) => {
//         setSelectedBooking(item); // Save the selected booking object
//         setBookingNumber(item.booking_number || ""); // Populate the current booking number
//         setStatus(item.status || ""); // Populate the current status
...
//                     <TouchableOpacity 
//                         style={styles.updateButton}
//                         onPress={() => updateBooking(selectedBooking.id)} // Update on press
//                     >
//                         <Text style={styles.updateButtonText}>Update</Text>
//                     </TouchableOpacity>
//                 </View>
...
//     return (
//         <ImageBackground source={require('../../assets/images/greens.png')} style={styles.backgroundContainer}>
//             {renderHeader()}
//             <FlatList
//                 data={bookings}
//                 renderItem={renderBookingItem}
