...
const CloseAppointment = ({navigation}) => {
        ...
        __getApiData('/api/booking/my_bookings')
            .then((res) => {
                if (res?.response?.response_code === "200" && res?.data?.aaData) {
                    // Filter bookings to show only those with status '0' (Pending)
                    const pendingBookings = res?.data?.aaData.filter(item => item.status === '2');
                    setBookings(pendingBookings); // Set filtered bookings
                    setError(null);
                } else {
    ...
    const renderHeader = () => {
        return (
            <View style={styles.headerContainer}>
                ...
                />
                <Text style={styles.headerTitle}>My Closed Appointment</Text>
            </View>
        );
    };
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyContainer}>
                        <Text style={styles.emptyText}>No close appointment available</Text>
                        <TouchableOpacity 
                            style={styles.retryButton} 
                            onPress={fetchBookings}
                        >
...
//                 </View>
//                 <View style={styles.line}/>
//                 <Text style={styles.patientName}>Patient Name: {item.customer_name || 'N/A'}</Text>
...
//                 data={bookings}
//                 renderItem={renderBookingItem}
//                 keyExtractor={(item) => item.booking_id?.toString() || item.booking_no.toString()}
//                 contentContainerStyle={styles.listContainer}
//                 ListEmptyComponent={
//                     <View style={styles.emptyContainer}>
