...
const AcceptedAppointment = ({ navigation }) => {
        ...
        __getApiData('/api/booking/my_bookings')
            .then((res) => {
                if (res?.response?.response_code === "200" && res?.data?.aaData) {
                    const pendingBookings = res?.data?.aaData.filter(item => item.status === '1');
    ...
    const renderHeader = () => {
        return (
            <View style={styles.headerContainer}>
                ...
                />
                <Text style={styles.headerTitle}>My Accepted Appointment</Text>
            </View>
        );
    };
    ...
    ListEmptyComponent={
        <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No Accepted appointment available</Text>
        </View>
    }
/>
 {BookingModal()}
