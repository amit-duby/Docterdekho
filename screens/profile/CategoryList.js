...
const CategoryList = ({
    ...
    return (
        <Overlay
            isVisible={showState}
            onBackdropPress={() => updateParentState({ showCategory: false })}
        ...
        const renderItem = ({ item }) => {
            return (
                <TouchableOpacity
                    ...
                    onPress={() => {
                        parantUpdateState({ category: item.id });
                        // __handleGetCity(item.state_id);
                        updateParentState({ showCategory: false });
                    }}
                >
                    <Feather
        ...
        return (
            <View style={{ ...styles.filterSheetTitleWrapStyle }}>
                ...
                <MaterialIcons
                    ...
                    size={22}
                    color={Colors.blackColor}
                    onPress={() => updateParentState({ showCategory: false })}
                    style={{
                        padding: 10,
                    }}
                />
